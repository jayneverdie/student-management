...
<section class="content">
  <div class="box box-primary">
    ...
    <div class="box-body">
		...
		<!-- grid -->
		<table id="grid_email_lists" class="table table-condensed table-striped" style="width:100%">
			<thead>
				<tr>
					<th>ID</th>
					<th>Customer Code</th>
      ...
      <div class="modal-body">
        ...
        <form id="form_new_emaillist">
          <div class="form-group">
            ...
            <input type="text" name="email_list" id="email_list" class="form-control" autofocus autocomplete="off" required>
          </div>
  ...
  <div class="modal-dialog modal-md" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <hr>
        <!-- grid -->
		<table id="grid_history" class="table table-condensed table-striped" style="width:100%">
			<thead>
				<tr>
					<th>ID</th>
					<th>Value</th>
...
<script>
  jQuery(document).ready(function ($) {
	...
	$('.--view-log').on('click', function () {
		...
		call_ajax('get', '/api/v1/logs/lists/column/'+table)
        .done(function(data) {
          $('#filter_column').html("<option value=''>- Select -</option>");
	        $.each(data, function(i, v) {
	        	$('#filter_column').append("<option value='" + v.LogsColumn + "'>"+ v.LogsColumn + "</option>");
	        });
	...
	loadGrid({
		...
		columnDefs: [
			{
				render: function(data, type, row) {
					return '<a href="javascript:void(0)" class="--col-customer-code" data-pk="'+row.ID+'">'+isNull(data)+'</a>';
				}, targets: 1
			},
			{
				render: function(data, type, row) {
					return '<a href="javascript:void(0)" class="--col-email" data-pk="'+row.ID+'">'+isNull(data)+'</a>';
				}, targets: 2
			},
			{
				render: function(data, type, row) {
					return '<a href="javascript:void(0)" class="--col-port" data-pk="'+row.ID+'">'+isNull(data)+'</a>';
				}, targets: 3
			},
			{
				render: function(data, type, row) {
					return '<a href="javascript:void(0)" class="--col-email-type" data-pk="'+row.ID+'">'+isNull(data)+'</a>';
				}, targets: 4
			},
			{
				render: function(data, type, row) {
					return '<a href="javascript:void(0)" class="--col-project" data-pk="'+row.ID+'">'+isNull(data)+'</a>';
				}, targets: 5
			},
			{
				render: function(data, type, row) {
					return '<a href="javascript:void(0)" class="--col-category" data-pk="'+row.ID+'">'+isNull(data)+'</a>';
				}, targets: 6
			},
			{
				render: function(data, type, row) {
					return '<a href="javascript:void(0)" class="--col-status" data-pk="'+row.ID+'">'+isNull(data)+'</a>';
				}, targets: 7
			}
		]
	});
