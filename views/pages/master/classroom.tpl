<?php $this->layout('layouts/dashboard', ['title' => 'Master-classroom']);?>
      ...
      <div class="modal-body">
        ...
        <form id="form_create">
          <div class="form-group">
            <label for="name_classroom">สถานะความสัมพันธ์</label>
            <input type="text" name="name_classroom" id="name_classroom" class="form-control" autocomplete="off" autofocus required>
          </div>
...
<script>
  jQuery(document).ready(function ($) {
    ...
    var grid_classroom_callback = function() {
      // cap name
      $('#grid_classroom .--classroom-name').editable({
        type: 'text',
        name: 'name',
        url: '/api/v1/master/classroom/update',
        title: 'Name',
        success: function(response, newValue) {
          ...
          reloadGrid('#grid_classroom', grid_classroom_callback);
        }
      });
    };
    ...
    loadGrid({
      ...
      modeSelect: "single",
      ajax: {
        url: '/api/v1/master/classroom/all',
        method: 'post'
      },
      ...
      columnDefs: [
        {
          render: function(data, type, row) {
            return '<a href="javascript:void(0)" class="--classroom-name" data-pk="'+row.id+'">'+data+'</a>';
          }, targets: 1
        }
      ]
    });
    ...
    $('#delete').on('click', function() {
      var rowdata = rowSelected('#grid_classroom');
      if (rowdata.length !== 0) {
        if (confirm('Aru you sure?')) {
          call_ajax('post', '/api/v1/master/classroom/delete', {
            id: rowdata[0].id
          }).done(function(data) {
            if (data.result === true) {
              reloadGrid('#grid_classroom', grid_classroom_callback);
    ...
    $('#form_create').submit(function(e) {
      e.preventDefault();
      call_ajax('post', '/api/v1/master/classroom/create', {
        name_classroom: $('#name_classroom').val()
      }).done(function(data) {
        if ( data.result === true ) {
